...
void MainWindow::slotFrameProcessed(const VideoProcessor& videoProcessor)
{
    ...
    mostrarEnfoque(videoProcessor.lastFocusScore, videoProcessor.parameters.focusThreshold, this->ui->video->sizeHint().width());

    int irisScore = ((status >= VideoProcessor::FOCUSED_NO_IRIS) ? videoProcessor.lastIrisQuality : 0);
    this->ui->irisScore->setValue(irisScore);
    this->ui->statusBar->showMessage(MainWindow::statusToString(videoProcessor));
}
...
QString MainWindow::statusToString(const VideoProcessor& videoProcessor)
{
    switch (videoProcessor.lastStatus) {
