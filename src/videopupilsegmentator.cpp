...
VideoPupilSegmentator::VideoPupilSegmentator()
{
...
VideoPupilSegmentator::VideoPupilSegmentator(VideoPupilSegmentatorParameters params) :
...
ContourAndCloseCircle VideoPupilSegmentator::segmentPupil(const GrayscaleImage& image)
{
...
void VideoPupilSegmentator::setupBuffers(const Image& image)
{
...
Circle VideoPupilSegmentator::approximatePupil(const GrayscaleImage& image)
{
    ...
    this->postprocessSimilarity(this->similarityImage);
...
Contour VideoPupilSegmentator::adjustPupilContour(const GrayscaleImage& image, const Circle& approximateCircle)
{
...
Circle VideoPupilSegmentator::cascadedIntegroDifferentialOperator(const GrayscaleImage& image)
{
...
VideoPupilSegmentator::MaxAvgRadiusResult VideoPupilSegmentator::maxAvgRadius(const GrayscaleImage& image, int x, int y, int radmin, int radmax, int radstep)
{
...
uint8_t VideoPupilSegmentator::circleAverage(const GrayscaleImage& image, int xc, int yc, int rc)
{
...
void VideoPupilSegmentator::similarityTransform(const GrayscaleImage& src, GrayscaleImage& dest)
{
...
void VideoPupilSegmentator::postprocessSimilarity(GrayscaleImage& similarity)
{
    // Usually, on video, the borders have black edges which correspond to the
    // background. Give more importance to the center of the image by applying
    // a weighting factor with respect to the center of the image.
    int xcenter = similarity.cols/2, ycenter = similarity.rows/2;
    float maxDistance2 = xcenter*xcenter + ycenter*ycenter;
    for (int y = 0; y < similarity.rows; y++) {
        for (int x = 0; x < similarity.cols; x++) {
            float dx = x-xcenter;
            float dy = y-ycenter;
            float distanceToCenter2 = dx*dx + dy*dy;
            float factor = 1.0f-distanceToCenter2/maxDistance2;
            similarity(y, x) = factor * factor * (float)similarity(y, x);
        }
    }
    //imshow("SIM", similarity);
}

int VideoPupilSegmentator::calculatePupilContourQuality(const GrayscaleImage& region, const Mat_<uint16_t>& regionGradient, const Mat_<float>& contourSnake)
{
